<script lang="ts">
	export let gradientColorStart: string = "rgb(var(--color-primary-500))";
	export let gradientColorEnd: string = "rgb(var(--color-success-500))";
	export let width: number = 32;
	export let height: number = 32;
</script>

<svg {width} {height} viewBox="0 0 16 18" fill="none">
	<path
		d="M12.6 2.82C11.9164 2.03962 11.5397 1.03743 11.54 0H8.44997V12.4C8.42614 13.071 8.14284 13.7066 7.65974 14.1729C7.17663 14.6393 6.53142 14.8999 5.85997 14.9C4.43997 14.9 3.25997 13.74 3.25997 12.3C3.25997 10.58 4.91997 9.29 6.62997 9.82V6.66C3.17997 6.2 0.159973 8.88 0.159973 12.3C0.159973 15.63 2.91997 18 5.84997 18C8.98997 18 11.54 15.45 11.54 12.3V6.01C12.793 6.90985 14.2973 7.39265 15.84 7.39V4.3C15.84 4.3 13.96 4.39 12.6 2.82Z"
		fill="url(#paint0_linear_314_20)"
	/>
	<defs>
		<linearGradient
			id="paint0_linear_314_20"
			x1="15.84"
			y1="9.15786"
			x2="0.159973"
			y2="9.15786"
			gradientUnits="userSpaceOnUse"
		>
			<stop stop-color={gradientColorEnd} />
			<stop offset="1" stop-color={gradientColorStart} />
		</linearGradient>
	</defs>
</svg>
